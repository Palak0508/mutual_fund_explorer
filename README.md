...
* *Note: Calculations assume a standard Risk-Free Rate ($R_f$) proxy baseline of 6.5%.*

### 3. OLS Linear Regression (Alpha & Beta)
Regresses fund historical daily returns against the market baseline index to extract:
* **Beta ($\beta$)**: Systemic market sensitivity index.
* **Alpha ($\alpha$)**: Annualized excess returns generated purely via manager strategy selection.
